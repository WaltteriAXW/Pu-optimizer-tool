...
export class ExportService {
  ...
  private exportToCSV(results: CalculationResults, options: ExportOptions): string {
    ...
    if (compatibility) {
      ...
      rows.push(`Output,${compatibility.output_kg_min},kg/min`)
      rows.push(`Output Range Min,${compatibility.output_min_kg_min},kg/min`)
      rows.push(`Output Range Max,${compatibility.output_max_kg_min},kg/min`)
      rows.push(`Output In Range,${compatibility.output_in_range ? 'Yes' : 'No'},`)
      rows.push(`Line Demand,${compatibility.line_demand_bar},bar`)
      rows.push(`Injection Pressure,${compatibility.injection_pressure_bar},bar`)
      rows.push(`Injection Pressure Governed By,${compatibility.injection_pressure_governed_by},`)
  ...
  private exportToReport(results: CalculationResults, options: ExportOptions): string {
    ...
    if (compatibility) {
      ...
      lines.push(`OUTPUT:             ${compatibility.output_kg_min?.toFixed(2)} kg/min  (set this)`)
      lines.push(`  machine range:    ${compatibility.output_min_kg_min}-${compatibility.output_max_kg_min} kg/min`)
      lines.push(`Line demand:        ${compatibility.line_demand_bar} bar`)
      lines.push(`Injection pressure: ${compatibility.injection_pressure_bar} bar  (expected reading)`)
      lines.push(`  governed by:      ${compatibility.injection_pressure_governed_by}`)
